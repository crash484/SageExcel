...
const Charts = () => {
    ...
    useEffect(() => {
        const fetchVisuals = async () => {
            try {
                const response = await fetch('https://crash03-InternshipProjectBackend.hf.space/api/auth/getAnalysis', {
                    headers: {
    ...
    const handleDelete = async (id) => {
        try {
            const response = await fetch(`https://crash03-InternshipProjectBackend.hf.space/api/auth/analysis/${id}`, {
                method: 'DELETE',
                headers: {
                    Authorization: `Bearer ${token}`
                }
