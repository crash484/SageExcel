...
export default function History() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                    setIsLoading(true);
                    const response = await fetch('https://crash03-InternshipProjectBackend.hf.space/api/auth/getFiles', {
                        headers: {
    ...
    const handleDownload = async (fileId, fileName) => {
        ...
        try {
            const response = await fetch(`https://crash03-InternshipProjectBackend.hf.space/api/auth/download/${fileId}`, {
                method: 'GET',
                headers: {
                    Authorization: `Bearer ${token}`
                }
    ...
    const handleDelete = async (fileId) => {
        ...
        try {
            const response = await fetch(`https://crash03-InternshipProjectBackend.hf.space/api/auth/delete/${fileId}`, {
                method: 'DELETE',
                headers: {
                    Authorization: `Bearer ${token}`
                },
